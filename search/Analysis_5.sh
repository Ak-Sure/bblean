#!/bin/bash
#SBATCH --job-name=BBlean5  # Job name
#SBATCH --mail-user=ak.surendran@ufl.edu  # Email for notifications
#SBATCH --mail-type=FAIL,END  # When to send email notifications
#SBATCH --error=BBlean5.err  # File to log errors
#SBATCH --output=BBlean5.out  # File to log standard output
...
python3.11 run_all_benchmarks.py --data-dir data --output-dir search/results/RDKIT5 --fp-type rdkit --k-values 1 5 10 50 100 --n-probe-values 1 2 4 8 16 32 64 --n-runs 1 --n-queries 100 --run-plots --threshold 0.5 --verbose --force-reload